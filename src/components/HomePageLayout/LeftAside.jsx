...
const LeftAside = () => {
  const { newsArray, categories, isLoading, setCatId } = useAppContext();
        ...
        {!isLoading &&
          categories?.map((category, index) => (
            <NavLink
              ...
              key={index}
            >
              <button className=" text-nowrap max-lg:text-sm">
                {category?.name}
              </button>
            </NavLink>
          ))}
        ...
        {newsArray?.map((item, index) => {
          if (!item?.others_info.is_trending) {
            return (
              <div
                key={index}
                className=" border text-sm shadow-lg  flex flex-col items-center"
              >
